...
import com.huawei.rtc.rule.common.Constants;
import com.huawei.rtc.rule.common.OperationFailedException;
import com.huawei.rtc.rule.call.helper.CallHelper;
...
import org.apache.camel.component.ahc.HttpProtocolHeaderFilterStrategy;
 ...
 */
public class CallBinding implements Binding {
    protected final Logger log = LoggerFactory.getLogger(this.getClass());
    protected HeaderFilterStrategy httpProtocolHeaderFilterStrategy = new HttpProtocolHeaderFilterStrategy();

    public Request prepareRequest(CallEndpoint endpoint, Exchange exchange) throws CamelExchangeException {
        ...
        try {
            ...
            String url = CallHelper.createURL(exchange, endpoint);
            URI uri = CallHelper.createURI(exchange, url, endpoint);
    ...
    protected void populateHeaders(RequestBuilder builder, CallEndpoint endpoint, Exchange exchange) {
    ...
    protected void populateBody(RequestBuilder builder, CallEndpoint endpoint, Exchange exchange) throws CamelExchangeException {
        ...
        if (body == null) {
            try {
                ...
                if (data != null) {
                    if (contentType != null && Constants.CONTENT_TYPE_JAVA_SERIALIZED_OBJECT.equals(contentType)) {

                        if (!endpoint.getComponent().isAllowJavaSerializedObject()) {
                            throw new CamelExchangeException("Content-type " + Constants.CONTENT_TYPE_JAVA_SERIALIZED_OBJECT + " is not allowed", exchange);
                        }
                        ...
                        CallHelper.writeObjectToStream(bos, obj);
    ...
    @Override
    public void onThrowable(CallEndpoint endpoint, Exchange exchange, Throwable t) throws Exception {
        exchange.setException(t);
    }

    @Override
    public void onStatusReceived(CallEndpoint endpoint, Exchange exchange, HttpResponseStatus responseStatus) throws Exception {
    ...
    @Override
    public void onHeadersReceived(CallEndpoint endpoint, Exchange exchange, HttpResponseHeaders headers) throws Exception {
    ...
    @Override
    public void onComplete(CallEndpoint endpoint, Exchange exchange, String url, ByteArrayOutputStream os, int contentLength,
        ...
        if (contentType != null) {
            ...
            CallHelper.setCharsetFromContentType(contentType, exchange);
        }
        ...
        if (contentType != null && contentType.equals(Constants.CONTENT_TYPE_JAVA_SERIALIZED_OBJECT)) {
            if (endpoint.getComponent().isAllowJavaSerializedObject() || endpoint.isTransferException()) {
                body = CallHelper.deserializeJavaObjectFromStream(is);
            }
        }
    ...
    private Exception populateHttpOperationFailedException(CallEndpoint endpoint, Exchange exchange, String url,
        ...
        if (statusCode >= 300 && statusCode < 400) {
            ...
            if (redirectLocation != null) {
                answer = new OperationFailedException(url, statusCode, statusText, redirectLocation, headers, copy);
            } else {
                // no redirect location
                answer = new OperationFailedException(url, statusCode, statusText, null, headers, copy);
            }
        } else {
